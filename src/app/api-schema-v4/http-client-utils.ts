...
export function GET<T = any>(options: FullHttpOptions) {
  ...
  return <R = any>(paramsFn: () => T = DEFAULT_GET_PARAMS_FN) => {
    return rxResource({
      request: paramsFn,
      loader: ({ request: params }) => httpRequest(APIClient.GET, options, params) as Observable<R>
    });
  };
}

export function POST<B = any, T = any>(options: ExtendedFullHttpOptions) {
  APIClient.assertHttpClient();
  return <R = any>(paramsFn: () => { body?: B, params?: T } = DEFAULT_POST_PARAMS_FN) => {
    return rxResource({
      request: paramsFn,
      loader: ({ request: { body, params } }) => httpBodyRequest(APIClient.POST, options, params, body) as Observable<R>
    });
  };
}

export function PUT<B = any, T = any>(options: ExtendedFullHttpOptions) {
  APIClient.assertHttpClient();
  return <R = any>(paramsFn: () => { body?: B, params?: T } = DEFAULT_POST_PARAMS_FN) => {
    return rxResource({
      request: paramsFn,
      loader: ({ request: { body, params } }) => httpBodyRequest(APIClient.PUT, options, params, body) as Observable<R>
    });
  };
}
...
export function DELETE<T = any>(options: FullHttpOptions) {
  ...
  return <R = any>(paramsFn: () => T = DEFAULT_GET_PARAMS_FN) => {
    return rxResource({
      request: paramsFn,
      loader: ({ request: params }) => httpRequest(APIClient.DELETE, options, params) as Observable<R>
    });
  };
}
